...
class memnode :
    ...
    def evaluate(self,s):
        ifnode.memory[0]=self.v
        ifnode.memory[len(ifnode.memory)-2]=deepcopy(ifnode.snakeloc)
        ifnode.memory[len(ifnode.memory)-1]=ifnode.food
        return self.v
...
def dangerBeforeFoodAhead(snakeloc,obstacles,food):
    if foodAhead(snakeloc,obstacles,food) is False : return False
    obs=obstacles+snakeloc[1:]
    direction=getSnakeDirection(snakeloc,obstacles,food)
    for o in obs:
        if direction is 0 :
            if (snakeloc[0][1]==o[1])and(o[0]>snakeloc[0][0]):return True
        elif direction is 1 :
            if (snakeloc[0][0]==o[0])and(o[1]<snakeloc[0][1]):return True
        elif direction is 2 :
            if (snakeloc[0][1]==o[1])and(o[0]<snakeloc[0][0]):return True
        else :
            if (snakeloc[0][0]==o[0])and(o[1]>snakeloc[0][1]):return True
dbfaw=fwrapper(dangerBeforeFoodAhead,0,'dangerBeforeFoodAhead')

def dangerBeforeFoodLeft(snakeloc,obstacles,food):
    if foodLeft(snakeloc,obstacles,food) is False : return False
    obs=obstacles+snakeloc[1:]
    direction=getSnakeDirection(snakeloc,obstacles,food)
    for o in obs:
        if direction is 0 :
            ...
            if (snakeloc[0][1]==o[1])and(o[0]>snakeloc[0][0]):return True
dbflw=fwrapper(dangerBeforeFoodLeft,0,'dangerBeforeFoodLeft')

def dangerBeforeFoodRight(snakeloc,obstacles,food):
    if foodRight(snakeloc,obstacles,food) is False : return False
    obs=obstacles+snakeloc[1:]
    direction=getSnakeDirection(snakeloc,obstacles,food)
    for o in obs:
        if direction is 2 :
            ...
            if (snakeloc[0][1]==o[1])and(o[0]>snakeloc[0][0]):return True
dbfrw=fwrapper(dangerBeforeFoodRight,0,'dangerBeforeFoodRight')

def dangerTwoLeft(snakeloc,obstacles,food):
    #return getLeftObstacleDistance(snakeloc,obstacles,food)==1
    obs=obstacles+snakeloc[1:]
    head=snakeloc[0]
    direction=getSnakeDirection(snakeloc,obstacles,food)
    if direction is 0 : return (head[0],head[1]-30) in obs
    elif direction is 1: return (head[0]-30,head[1]) in obs
    elif direction is 2: return (head[0],head[1]+30) in obs
    else : return (head[0]+30,head[1]) in obs
dtlw=fwrapper(dangerTwoLeft,0,'dangerTwoLeft')

def dangerTwoRight(snakeloc,obstacles,food):
    #return getRightObstacleDistance(snakeloc,obstacles,food)==1obs=obstacles+snakeloc[1:]
    obs=obstacles+snakeloc[1:]
    head=snakeloc[0]
    direction=getSnakeDirection(snakeloc,obstacles,food)
...
dtrw=fwrapper(dangerTwoRight,0,'dangerTwoRight')

def dangerTwoAhead(snakeloc,obstacles,food):
    #return getStraightObstacleDistance(snakeloc,obstacles,food)==1
    obs=obstacles+snakeloc[1:]
    head=snakeloc[0]
    direction=getSnakeDirection(snakeloc,obstacles,food)
    if direction is 0 : return (head[0]+30,head[1]) in obs
    elif direction is 1: return (head[0],head[1]-30) in obs
    elif direction is 2: return (head[0]-30,head[1]) in obs
    else : return (head[0],head[1]+30) in obs
dtaw=fwrapper(dangerTwoAhead,0,'dangerTwoAhead')

def memoryZero(snakeloc,obstacles,food):
    return ifnode.memory[0]==0
m0w=fwrapper(memoryZero,0,'memoryZero')

def memoryOne(snakeloc,obstacles,food):
    return ifnode.memory[0]==1
m1w=fwrapper(memoryOne,0,'memoryOne')

def memoryTwo(snakeloc,obstacles,food):
    return ifnode.memory[0]==2
m2w=fwrapper(memoryTwo,0,'memoryTwo')

def sameInfo(snakeloc,obstacles,food):
    return ifnode.memory[len(ifnode.memory)-2]==snakeloc and ifnode.memory[len(ifnode.memory)-1]==food
siw=fwrapper(sameInfo,0,'sameInfo')



flist=[ifw,gtw,addw,subw,mulw]
fnlist=[vfdw,hfdw,lodw,rodw,sodw]
condlist=[dlw,drw,daw,faw,frw,flw,mrw,mlw,muw,mdw,fuw,ftrw,dbflw,dbfrw,dbfaw,m0w,m1w,m2w,siw]
...
def makerandomtree(pc,maxdepth=depth,fpr=0.6,ppr=0,fnpr=0.4):    # default fpr=0.5,ppr=0.6
    ...
    elif random.random()<fnpr and maxdepth>0:
        return memnode(random.choice([0,1,2]))
    elif random.random()<fnpr:
        return execmemnode()
    else :
...
def mutate(t,pc,height=depth,probchange=0.1):
    if random.random()<probchange:
        return makerandomtree(pc,height)
    else:
        result=deepcopy(t)
    if isinstance(t,ifnode) and height > 0:
        result.children=[mutate(c,pc,height-1,probchange) for c in t.children]
    return result
...
def reachable_coords(obstacles):
    queue=[(0,0)]
    reachables=[]
    while len(queue)>0:
        v=queue[0]
        ...
        n=[v1,v2,v3]
        for u in n:
            if u not in obstacles and u[0]<300 and u[1]<300 and u not in queue and u not in reachables:
                queue+=[u]
    removed=True
    while removed is True:
        ...
        for v in reachables:
            ...
            v3=(v[0],v[1]+15)
            v4=(v[0],v[1]-15)
            count=reachables.count(v1)+reachables.count(v2)+reachables.count(v3)+reachables.count(v4)
            if count<2:
                removed=True
                reachables.remove(v)
    return reachables
...
def get_random_foodloc(reachables):
    return random.choice(reachables)

def get_random_snake(size,pc):
    return snake(get_random_snake_coords(2),makerandomtree(pc,depth))
...
def scorefunction(snake_,foods,obstacles=[],moves=1000,spacing=15) :
    ...
    for j in range(moves) :
        ...
        if head==food :
            ...
            if foodindex is 0 and (head[0]<0 or head[0]>=300 or head[1]<0 or head[1]>=300): return score-5000000
...
def getPotential(foods,moves):
    foodindex=0
    potential=0
    while moves>0:
        moves-=(int)((math.fabs(foods[foodindex][0]-foods[foodindex+1][0])+math.fabs(foods[foodindex][1]-foods[foodindex+1][1]))/15)
        potential+=10000
        foodindex+=1
    return potential
...
def rankfunction(population,obstacles=[],moves=1000) :
    ...
    #obs=[]
    #reachables=[]
    #while len(reachables) < 330:
    #    obs=[]
    #    for i in range(3):obs+=get_random_obstacle_coords(15)
    #for o in obs :
    ...
    #for i in range(3):obs+=get_random_obstacle_coords(10)
    reachables=[]
    while len(reachables)<325:
        obs=[]
        for i in range(3):obs+=get_random_obstacle_coords(10)
        for o in obs :
            if o[0]<75 or o[1]<75 : obs.remove(o)
        obs+=obstacles
        reachables=reachable_coords(obs)
    #obs=deepcopy(obstacles)
    #reachables=reachable_coords(obs)
    foods=[get_random_foodloc(reachables) for m in range(moves)]
    ...
    #start=True
    i=0
    for snake_ in population :
        ifnode.memory=[0]+[[],(-1,-1)]
        scores+=[scorefunction(snake_,foods,obs,moves)]
        i+=1
        print("\rNo of snake simulations completed : %d"%(i)),
    ...
    return scores,getPotential(foods,moves)
...
def change_canvas(snake,prev_snake_indices,food,obstacles=[],spacing=15):
    ...
    if head==food :
        ...
        idx=canvas.create_rectangle(snake_head[0]+4,snake_head[1]+4,snake_head[0]+spacing-4,snake_head[1]+spacing-4,fill='orange')
        snake_indices+=[idx]
        food=get_random_foodloc(reachable_coords(obstacles))
...
def evolve(pc,popsize,maxgen=500,obstacles=[],mutationrate=0.1,breedingrate=0.4,pexp=0.7,pnew=0.5):
    ...
    for gen in range(maxgen) :
        ...
        scores,potential=rankfunction(population,obstacles,5000)
        ...
        scores.sort()
        scores.reverse()
        print "\rGeneration",gen,"| Best scores obtained =",scores[0],"-",scores[4],"Potential :",potential
        ...
        while len(population)<popsize :
            ...
            print("\rNo of snakes in new population : %d"%(len(population))),
        del sorted_snakes
...
def simulate_saved_snake(obstacles):
    data_file=open("GP Data/GP Run - 3/Best snake.txt","r")
    snake_=pickle.load(data_file)
    data_file.close()
    foodloc=get_random_foodloc(reachable_coords(obstacles))
    print foodloc
    print foodloc in obstacles
...
evolve(5,1000,1000,obstacles,0.2,0.5)
#for i in range(3):
#    obstacles+=get_random_obstacle_coords(15)
#print len(reachable_coords(obstacles))
