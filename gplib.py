...
condlist=[dlw,drw,daw,faw,frw,flw,dbfaw,fuw,ftrw]
...
def scorefunction(snake_,foods,obstacles=[],moves=1000,spacing=15) :
    ...
    for j in range(moves) :
        ...
        if head==food :
            ...
            if score is 0 and (head[0]<0 or head[0]>=300 or head[1]<0 or head[1]>=300): return -500000
...
def rankfunction(population,obstacles=[],moves=1000) :
    ...
    while len(reachables) < 330:
        obs=[]
        for i in range(0):obs+=get_random_obstacle_coords(1)
        obs+=obstacles
...
def simulate_saved_snake(obstacles):
    data_file=open("GP Data/GP Run - 81/Best snake.txt","r")
